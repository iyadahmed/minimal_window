                            ...
                            uint32_t version)
{
    ...
    {
        struct wl_seat *seat = wl_registry_bind(global_client_state.wl_registry,
...
void nano_gui_create_fixed_size_window(int width, int height)
{
    ...
    global_client_state.wl_registry =
        wl_display_get_registry(global_client_state.wl_display);
    wl_registry_add_listener(global_client_state.wl_registry,
                             &wl_registry_listener, &global_client_state);
    wl_display_roundtrip(global_client_state.wl_display);

    global_client_state.wl_surface =
        wl_compositor_create_surface(global_client_state.wl_compositor);
    global_client_state.xdg_surface = xdg_wm_base_get_xdg_surface(
        global_client_state.xdg_wm_base, global_client_state.wl_surface);
    xdg_surface_add_listener(global_client_state.xdg_surface,
                             &xdg_surface_listener, &global_client_state);
    global_client_state.xdg_toplevel =
        xdg_surface_get_toplevel(global_client_state.xdg_surface);
    xdg_toplevel_set_title(global_client_state.xdg_toplevel, "Example client");
    wl_surface_commit(global_client_state.wl_surface);

    xdg_toplevel_add_listener(global_client_state.xdg_toplevel,
    ...
    global_image_data =
        mmap(NULL, global_data_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    struct wl_shm_pool *pool =
        wl_shm_create_pool(global_client_state.wl_shm, fd, global_data_size);
    global_wl_buffer = wl_shm_pool_create_buffer(pool, 0, width, height, stride,
